...
public class VPestaniasAgente extends JFrame implements ActionListener, MouseListener {
	...
	private JCheckBox chckbxRegisterAgentModif;
	...
	private JCheckBox chckbxRegisterAgent;
	...
	public VPestaniasAgente(Agent loginAgent, MapManager map, AgentManager agent, WeaponManager weapon,
		...
		chckbxRegisterAgent = new JCheckBox("Admin?");
		chckbxRegisterAgent.setOpaque(false);
		chckbxRegisterAgent.setFont(new Font("DINNextLTPro-Regular", Font.BOLD, 14));
		chckbxRegisterAgent.setBounds(151, 395, 182, 23);
		panelRegisterAgent.add(chckbxRegisterAgent);
		...
		chckbxRegisterAgentModif = new JCheckBox("Admin?");
		chckbxRegisterAgentModif.setFont(new Font("DINNextLTPro-Regular", Font.BOLD, 14));
		chckbxRegisterAgentModif.setOpaque(false);
		chckbxRegisterAgentModif.setBounds(151, 395, 182, 23);
		panelModifyAgent.add(chckbxRegisterAgentModif);
		...
		spinnerModif.setModel(new SpinnerNumberModel(5, 5, 8, 1));
		...
		comboBoxWeaponType.setModel(
				new DefaultComboBoxModel<String>(new String[] { "Todas las armas", "Primarias", "Secundarias" }));
		...
		comboBoxMissions = new JComboBox<String>();
		...
		listMission.setBounds(630, 165, 510, 714);
		...
		btnConfirmMission = new JButton("Añadir mision");
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource().equals(btnClose)) {
			...
			chckbxRegisterAgentModif.setEnabled(false);
			if (newAgent.isAgentIsAdmin()) {
				chckbxRegisterAgentModif.setSelected(true);
			} else {
				chckbxRegisterAgentModif.setSelected(false);
			}
			...
			if (txtCodeModif.getText().isEmpty() || txtNationalityModif.getText().isEmpty()
				...
				newAgent.setAgentIsAdmin(chckbxRegisterAgent.isSelected());
		...
		if (txtName.getText().isEmpty() || txtCode.getText().isEmpty() || txtNationality.getText().isEmpty()
			...
			if (passwd1.equals(passwd2)) {
				...
				try {
					...
					cargarTablaAgents(p);
					JOptionPane.showMessageDialog(this, "Agente creado correctamente", "Agente",
							JOptionPane.INFORMATION_MESSAGE);
				} catch (ExceptionManager e) {
		...
		if (e.getSource().equals(table)) {
			if (e.getClickCount() == 2) {
				try {
					...
					if (newAgent.isAgentIsOnActive()) {
						if (JOptionPane.showOptionDialog(this, "¿Desea dar de baja al agente?",
						...
						if (JOptionPane.showOptionDialog(this, "¿Desea dar de alta al agente?",
								newAgent.getAgentName() + " " + newAgent.getAgentCode(), JOptionPane.YES_NO_OPTION,
	...
	private void clean() {
		...
		comboBoxRol.setSelectedIndex(-1);
		chckbxRegisterAgent.setSelected(false);
		txtHabilityName1.setText("");
		textAreaHabilityDescription1.setText("");
		txtHabilityName2.setText("");
		textAreaHabilityDescription2.setText("");
		txtHabilityName3.setText("");
		textAreaHabilityDescription3.setText("");
		txtHabilityName4.setText("");
		textAreaHabilityDescription4.setText("");
		txtPasswd1.setText("");
		txtPasswd2.setText("");
	}
}
